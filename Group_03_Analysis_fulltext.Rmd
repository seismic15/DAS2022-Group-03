...
household %>%
    dplyr::select(Members.no, Income, Food.exp, Head.age, Area, House.age, Bedrooms.no) %>%
    cor() %>%
    # Code reference:
...
```
From the Figure \ref{fig:roc}, the AUC value is 0.812, which is larger than 0.5. So, it is better than the predicting randomly. We can adjust the threshold to calculate the accuracy and when we set the threshold as 0.51, we can see the accuracy is 70.6%. 
