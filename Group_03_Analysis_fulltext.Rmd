...
# Add mirror for install.packages
# Knitr produces a R session, without a default cran mirror. 
...
According to several analyses, we find the best model to fit the dataset. The number of members in household is divided into 2 groups: 1-4 members and more than 5 members. 2 variables are excluded: household area and the number of bedrooms. Then there are 7 variables: annual household income, Annual expenditure by the household on food, head of the households sex, head of the households age, relationship between the group of people living in the house, age of the building, and whether the house have electricity are the final variables included. They influence the number of members living in a household.

Among these 7 variables, food expenditure has the strongest influence and total income has the second strongest influence. By comparison, the house age and head age only influence the outcomes slightly. The other 3 indicator variables are contribute moderately. If household has properties: non-electricity, male head, and more than one family, then there is a higher probability that the house includes more than 5 members in a household.
...
household %>%
    mutate(log.Member.no = log(Members.no)) %>%
    dplyr::select(log.Member.no, Income, Food.exp, Head.age, Area, House.age, Bedrooms.no) %>%
    cor() %>%
    # Code reference:
...
points0 <- c(1, 4, 8, 12, 16)
household$quantiles <- cut(household$Members.no, breaks = points0,
                           include.lowest = TRUE)

household %>% group_by(quantiles) %>% count() %>%
  knitr::kable(caption = '\\label{tab:multi} Four-group classification', linesep = "", digits = 2) %>%
  kableExtra::kable_styling(latex_options = 'HOLD_position')
...
$$
\log(\frac{p_i}{1-p_i}) = \alpha + \beta_1 \log(x_{1i}) +\beta_2 log(x_{2i}) +\beta_3 x_{3i} + \beta_4 x_{4i} + \beta_5 \mbox{I}(x_{5i}) + \beta_6 \mbox{I}(x_{6i}) + \beta_7 \mbox{I}(x_{7i}) + \epsilon_i,
$$
where 
...
- $x_7$ denotes the **Household type**
...
```{r, fig.width = 8, fig.height = 9, fig.align = "center", fig.cap = "\\label{fig:prob1}The probability of more members (Part 1)", fig.pos = "H"}
...
```
From the plot (a) in Figure \ref{fig:prob1}, the probability of members declines with the increase of household income. At first, it declines steeply and gradually become steady, while in contrast, from the plot (b), which is the plot of food expenditure, we witness that the line increases dramatically before 500,000 peso and then become flat. In plot (c) and (d), the slope of head of household age and house age decrease with steady paces. Besides, plot (e), (f) and (g) are plot of three categorical variables. Finally, we can conclude that more expenditure, houses with no electricity, male head of household and single type of relationship will lead to more family members. 


```{r, fig.width = 4, fig.height = 3, fig.align = "center", fig.cap = "\\label{fig:prob2}The probability of more members (Part 2)", fig.pos = "H"}
...
$$
\log(\frac{p_i}{1-p_i}) = \alpha + \beta_1 \log(x_{1i}) +\beta_2 log(x_{2i}) +\beta_3 x_{3i} + \beta_4 x_{4i} + \beta_5 \mbox{I}(x_{5i}) + \beta_6 \mbox{I}(x_{6i}) + \beta_7 \mbox{I}(x_{7i}) + \beta_8 \log(x_{8i}) + \beta_9 x_{9i} + \epsilon_i,
$$
where 
...
Take the poisson model as a example, all the p values of these variables shows that they are significant. Therefore, we next check its residuals and assumptions:
...
# Conclusion
According to several analyses, we find the best model to fit the dataset. In summary, there are 7 variables that could influence the number of people living in a household:
